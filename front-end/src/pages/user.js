...
export default function User() {
    ...
    useEffect(()=>{
        rents.forEach(rent => {
            ...
            .then(data=>{
                rendergames =  <div className="rounded-2xl border-4 border-stone-800 p-3 bg-cyan-400 mx-2 mt-3 mb-5 font-mainFont text-stone-800 col-span-1 md:col-span-1 lg:col-span-1 grid grid-cols-1">
                                <h2 className="text-xs mb-2">{data.gameId}</h2>
                                <h1 className="text-lg">{data.gameName}</h1>
                                <h2 className="text-xs">{data.launchDate}</h2>
                                <h2 className="text-xs">{data.mainCharacter}</h2>
                                <h2 className="text-xs">{data.gameDirector}</h2>
                                <h2 className="text-xs">{data.producer}</h2>
                                <h2 className="text-xs">{data.gamePlatform}</h2>
                                <h2 className="text-xs">{data.gamePrice}</h2>
                                <h2 className="text-xs mt-2">Stock: {data.stock}</h2>
                            </div>
    ...
    renderRents = rents.map((rent, index) => {
        return  <div key={index} className="grid grid-cols-1 md:grid-cols-4 col-span-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 mb-5 col-span-1 md:col-span-4">
                        {games}
                    </div>
                </div>
    })
